...
def build_local_prompt(job: dict, tailored_resume: str,
                       cover_letter: str | None = None,
                       dry_run: bool = False) -> str:
    """Build a compact prompt for local LLMs (Ollama, llama.cpp).

    Much shorter than build_prompt(). Strips CAPTCHA instructions, verbose
    form tricks, and edge cases. Focuses on: navigate, fill form, submit.
    Designed to fit in 8-32K context windows.
    """
    ...
    if dry_run:
        submit_note = "DRY RUN: Do NOT click Submit. Review the form, then output RESULT:APPLIED."
    else:
        submit_note = "Before clicking Submit, review all fields. Then click Submit."

    prompt = f"""You are a job application agent. Fill out the application form and submit it.

APPLICATION URL: {apply_url}
JOB: {job['title']} at {job.get('site', 'Unknown')}

APPLICANT INFO:
- Name: {full_name}
- Email: {personal['email']}
- Phone: {personal['phone']} (digits only: {phone_digits})
- Address: {personal.get('address', '')}, {personal.get('city', '')}, {personal.get('province_state', '')}, {personal.get('country', '')}, {personal.get('postal_code', '')}
- LinkedIn: {personal.get('linkedin_url', 'N/A')}
- GitHub: {personal.get('github_url', 'N/A')}
- Work Authorization: {work_auth.get('legally_authorized_to_work', 'Yes')}
- Sponsorship Needed: {work_auth.get('require_sponsorship', 'No')}
- Salary Expectation: ${comp['salary_expectation']} {comp.get('salary_currency', 'USD')}
- Age 18+: Yes
- Background Check: Yes
- Felony: No

RESUME PDF (upload this file): {pdf_path}
{f'COVER LETTER PDF: {cl_upload_path}' if cl_upload_path else ''}

RESUME TEXT (use when filling text fields):
{tailored_resume[:3000]}

INSTRUCTIONS:
1. The browser is already on the application page. Call browser_snapshot to see the page.
2. Look for an Apply button and click it using browser_click with the ref number.
3. Upload the resume PDF using browser_file_upload when you find a file input.
4. Fill all form fields using browser_fill with ref numbers from the snapshot.
5. For dropdowns, use browser_select_option.
6. Answer screening questions truthfully using the applicant info above.
7. For open-ended questions, write 2-3 specific sentences about this role.
8. {submit_note}
9. Output your result code.

RESULT CODES (output exactly one when done):
RESULT:APPLIED - submitted successfully
RESULT:EXPIRED - job closed or not accepting applications
RESULT:FAILED:reason - any failure (brief reason)

TIPS:
- Call browser_snapshot to see interactive elements with [ref=N] tags.
- Use those ref numbers with browser_click, browser_fill, browser_select_option.
- If you see a login wall, try: {personal['email']} / {personal.get('password', '')}
- EEO/demographics: always "Decline to self-identify"
- If stuck on the same page after 3 tries, output RESULT:FAILED:stuck"""
...
def build_prompt(job: dict, tailored_resume: str,
    ...
    from applyagent.config import load_blocked_sso
