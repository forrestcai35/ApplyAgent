...
from applyagent.config import COVER_LETTER_DIR, RESUME_PATH, load_profile
from applyagent.database import get_connection, get_jobs_by_stage
from applyagent.llm import get_client
from applyagent.scoring.validator import (
    BANNED_WORDS,
    LLM_LEAK_PHRASES,
    sanitize_text,
...
def run_cover_letters(min_score: int = 7, limit: int = 20,
    ...
    for job in jobs:
        ...
        try:
            ...
            pdf_path = None
            try:
                from applyagent.scoring.pdf import convert_to_pdf
