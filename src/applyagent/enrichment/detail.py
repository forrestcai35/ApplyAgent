...
from applyagent.config import DB_PATH
from applyagent.database import get_connection, init_db, ensure_columns
from applyagent.llm import get_client
...
def set_proxy(proxy_str: str | None):
    ...
    if proxy_str:
        from applyagent.discovery.jobspy import parse_proxy
...
def _load_base_urls() -> dict[str, str | None]:
    ...
    from applyagent.config import load_base_urls
...
def extract_with_llm(page, url: str) -> dict:
    ...
    try:
        ...
        from applyagent.discovery.smartextract import extract_json
