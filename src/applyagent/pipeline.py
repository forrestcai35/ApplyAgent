"""ApplyAgent Pipeline Orchestrator.

Runs pipeline stages in sequence or concurrently (streaming mode).

Usage (via CLI):
    applyagent run                        # all stages, sequential
    applyagent run --stream               # all stages, concurrent
    applyagent run discover enrich        # specific stages
    applyagent run score tailor cover     # LLM-only stages
...
from applyagent.database import init_db, get_connection, get_stats
...
def _run_discover(workers: int = 1) -> dict:
    ...
    try:
        from applyagent.discovery.jobspy import run_discovery
        run_discovery()
        stats["jobspy"] = "ok"
    ...
    try:
        from applyagent.discovery.workday import run_workday_discovery
    ...
    try:
        from applyagent.discovery.smartextract import run_smart_extract
...
def _run_enrich(workers: int = 1) -> dict:
    ...
    try:
        from applyagent.enrichment.detail import run_enrichment
...
def _run_score() -> dict:
    ...
    try:
        from applyagent.scoring.scorer import run_scoring
        run_scoring()
        return {"status": "ok"}
    except Exception as e:
...
def _run_tailor(min_score: int = 7, validation_mode: str = "normal") -> dict:
    ...
    try:
        from applyagent.scoring.tailor import run_tailoring
...
def _run_cover(min_score: int = 7, validation_mode: str = "normal") -> dict:
    ...
    try:
        from applyagent.scoring.cover_letter import run_cover_letters
...
def _run_pdf() -> dict:
    ...
    try:
        from applyagent.scoring.pdf import batch_convert
        batch_convert()
        return {"status": "ok"}
    except Exception as e:
...
def run_pipeline(
    ...
    console.print(Panel.fit(
        f"[bold]ApplyAgent Pipeline[/bold] ({mode})",
        border_style="blue",
    ))
