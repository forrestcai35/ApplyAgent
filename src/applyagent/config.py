"""ApplyAgent configuration: paths, platform detection, user data."""
...
APP_DIR = Path(os.environ.get("APPLYAGENT_DIR", Path.home() / ".applyagent"))

# Core paths
DB_PATH = APP_DIR / "applyagent.db"
...
def load_profile() -> dict:
    """Load user profile from ~/.applyagent/profile.json."""
    import json
    if not PROFILE_PATH.exists():
        raise FileNotFoundError(
            f"Profile not found at {PROFILE_PATH}. Run `applyagent init` first."
        )
    return json.loads(PROFILE_PATH.read_text(encoding="utf-8"))


def load_search_config() -> dict:
    """Load search configuration from ~/.applyagent/searches.yaml."""
    import yaml
...
def load_env():
    """Load environment variables from ~/.applyagent/.env if it exists."""
