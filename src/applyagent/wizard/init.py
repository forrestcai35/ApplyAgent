"""ApplyAgent first-time setup wizard.

Interactive flow that creates ~/.applyagent/ with:
...
def _setup_profile() -> dict:
    ...
    console.print(Panel("[bold]Step 2: Profile[/bold]\nTell ApplyAgent about yourself. This powers scoring, tailoring, and auto-fill."))
...
def _setup_searches() -> None:
    ...
    # Build YAML content
    lines = [
        "# ApplyAgent search configuration",
...
def _setup_ai_features() -> None:
    ...
    if not Confirm.ask("Enable AI scoring and resume tailoring?", default=True):
        console.print("[dim]Discovery-only mode. You can configure AI later with [bold]applyagent init[/bold].[/dim]")
        return
    ...
    env_lines = ["# ApplyAgent configuration", ""]
...
def _setup_auto_apply() -> None:
    ...
    if not Confirm.ask("Enable autonomous job applications?", default=True):
        console.print("[dim]You can apply manually using the tailored resumes ApplyAgent generates.[/dim]")
        return
    ...
    if Confirm.ask("Configure CapSolver API key? (optional)", default=False):
        ...
        if ENV_PATH.exists():
            ...
            ENV_PATH.write_text(f"# ApplyAgent configuration\nCAPSOLVER_API_KEY={capsolver_key}\n", encoding="utf-8")
...
def run_wizard() -> None:
    ...
    console.print(
        Panel.fit(
            "[bold green]ApplyAgent Setup Wizard[/bold green]\n\n"
            "This will create your configuration at:\n"
            f"  [cyan]{APP_DIR}[/cyan]\n\n"
            "You can re-run this anytime with [bold]applyagent init[/bold].",
            border_style="green",
        )
    )
    ...
    from applyagent.config import get_tier, TIER_LABELS, TIER_COMMANDS
    ...
    unlock_hint = ""
    if tier == 1:
        unlock_hint = "\n[dim]To unlock Tier 2: configure an LLM API key (re-run [bold]applyagent init[/bold]).[/dim]"
    elif tier == 2:
